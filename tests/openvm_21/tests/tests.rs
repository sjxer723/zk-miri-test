use openvm_circuit::{
    arch::{ExecutionBus, SystemConfig},
    system::memory::{
        BOUNDARY_AIR_OFFSET, MERKLE_AIR_OFFSET, MemoryController, MemoryImage, OfflineMemory,
...
fn new_systemcomplex<F: PrimeField32>() {
    ...
    let range_bus = VariableRangeCheckerBus::new(RANGE_CHECKER_BUS, config.memory_config.decomp);
    let mut bus_idx_max = RANGE_CHECKER_BUS;

    let range_checker = SharedVariableRangeCheckerChip::new(range_bus);
    let memory_controller: openvm_circuit::system::memory::MemoryController<F> =
